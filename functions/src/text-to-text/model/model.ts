import {ComlinkWorkerInterface, createBergamotWorker, ModelRegistry} from '@ai-converse/browsermt';
...
export class TextToTextTranslationModel {
  ...
  async init(files: string[]) {
    const workerPath = require.resolve('@ai-converse/browsermt/build/bundled/worker.js');
    this.worker = createBergamotWorker(workerPath);

    const wasmBinary = fs.readFileSync(require.resolve('@ai-converse/browsermt/artifacts/bergamot-translator-worker.wasm'));

    await this.worker.importBergamotWorker(
      require.resolve('@ai-converse/browsermt/artifacts/bergamot-translator-worker.js'),
      wasmBinary
    );
