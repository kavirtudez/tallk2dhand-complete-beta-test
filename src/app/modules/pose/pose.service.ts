...
export class PoseService {
  ...
  drawBody(landmarks: PoseLandmark[], ctx: CanvasRenderingContext2D): void {
    ...
    drawing.drawConnectors(ctx, filteredLandmarks, this.holistic.POSE_CONNECTIONS, {color: '#0472fd'});
    drawing.drawLandmarks(ctx, filteredLandmarks, {color: '#0472fd', fillColor: '#0472fd'});
  }
  ...
  drawHand(
    ...
    drawing.drawConnectors(ctx, landmarks, this.holistic.HAND_CONNECTIONS, {color: '#0472fd'});
  ...
  drawFace(landmarks: PoseLandmark[], ctx: CanvasRenderingContext2D): void {
    drawing.drawConnectors(ctx, landmarks, this.holistic.FACEMESH_TESSELATION, {color: '#0472fd70', lineWidth: 1});
    drawing.drawConnectors(ctx, landmarks, this.holistic.FACEMESH_RIGHT_EYE, {color: '#0472fd'});
    drawing.drawConnectors(ctx, landmarks, this.holistic.FACEMESH_RIGHT_EYEBROW, {color: '#0472fd'});
    drawing.drawConnectors(ctx, landmarks, this.holistic.FACEMESH_LEFT_EYE, {color: '#0472fd'});
    drawing.drawConnectors(ctx, landmarks, this.holistic.FACEMESH_LEFT_EYEBROW, {color: '#0472fd'});
    drawing.drawConnectors(ctx, landmarks, this.holistic.FACEMESH_FACE_OVAL, {color: '#0472fd'});
    drawing.drawConnectors(ctx, landmarks, this.holistic.FACEMESH_LIPS, {color: '#0472fd'});
  }
  ...
  draw(pose: EstimatedPose, ctx: CanvasRenderingContext2D): void {
    ...
    if (pose.leftHandLandmarks) {
      this.drawHand(pose.leftHandLandmarks, ctx, '#0472fd', '#0472fd', '#0472fd');
    }

    if (pose.rightHandLandmarks) {
      this.drawHand(pose.rightHandLandmarks, ctx, '#0472fd', '#0472fd', '#0472fd');
    }
